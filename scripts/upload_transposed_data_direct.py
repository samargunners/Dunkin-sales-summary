"""
Upload parsed transposed data to Supabase (Direct - No Confirmation)

This script takes the parsed data and uploads it directly to Supabase.
"""
    ...
    try:
        ...
        if data['sales_summary']:
            ...
            insert_query = """
                ...
                ON CONFLICT (store, pc_number, date)
                DO UPDATE SET
                    gross_sales = EXCLUDED.gross_sales,
                    net_sales = EXCLUDED.net_sales,
                    dd_adjusted_no_markup = EXCLUDED.dd_adjusted_no_markup,
                    pa_sales_tax = EXCLUDED.pa_sales_tax,
                    dd_discount = EXCLUDED.dd_discount,
                    guest_count = EXCLUDED.guest_count,
                    avg_check = EXCLUDED.avg_check,
                    gift_card_sales = EXCLUDED.gift_card_sales,
                    void_amount = EXCLUDED.void_amount,
                    refund = EXCLUDED.refund,
                    void_qty = EXCLUDED.void_qty,
            ...
            for record in data['sales_summary']:
                try:
                    ...
                    if error_count <= 5:  # Only show first 5 errors
                        print(f"  ❌ Error uploading record: {record['store']} {record['date']} - {str(e)[:100]}")
        ...
        if data['tender_type_metrics']:
            ...
            insert_query = """
                ...
                ON CONFLICT (store, pc_number, date, tender_type)
                DO UPDATE SET
            ...
            for record in data['tender_type_metrics']:
                try:
                    ...
                    error_count += 1
                    if error_count <= 5:  # Only show first 5 errors
                        print(f"  ❌ Error uploading record: {record['store']} {record['date']} {record['tender_type']} - {str(e)[:100]}")
        ...
        if data['labor_metrics']:
            ...
            insert_query = """
                ...
                ON CONFLICT (store, pc_number, date, labor_position)
                DO UPDATE SET
                    reg_hours = EXCLUDED.reg_hours,
                    ot_hours = EXCLUDED.ot_hours,
                    total_hours = EXCLUDED.total_hours,
                    reg_pay = EXCLUDED.reg_pay,
            ...
            for record in data['labor_metrics']:
                try:
                    ...
                    error_count += 1
                    if error_count <= 5:  # Only show first 5 errors
                        print(f"  ❌ Error uploading record: {record['store']} {record['date']} {record['labor_position']} - {str(e)[:100]}")
        ...
        print("="*80)
        print(f"Sales Summary: {success_count if 'sales_summary' in data else 0} records")
        print(f"Tender Type Metrics: {success_count if 'tender_type_metrics' in data else 0} records")
        print(f"Labor Metrics: {success_count if 'labor_metrics' in data else 0} records")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print("Usage: python upload_transposed_data_direct.py <path_to_file>")
        print("\nExample: python upload_transposed_data_direct.py /Users/samarpatel/Downloads/october_data.xlsx")
        sys.exit(1)
